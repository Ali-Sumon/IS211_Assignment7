...
class PigGame:
    ...
    def play(self):
        ...
        while True:
            print("\n" + "=" * 20)
            print(
                f"It's {self.players[self.current_player].name}'s turn. Current score: {self.current_score}, Total score: {self.players[self.current_player].score}")
            ...
            # If the winning score isn't reached, the next player method is called
                ...
                self.players[self.current_player].score += self.current_score
                print(
                    f"{self.players[self.current_player].name} banked {self.current_score} points. Total score: {self.players[self.current_player].score}")
                self.current_score = 0
